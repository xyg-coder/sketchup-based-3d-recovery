...
public:
	...
	*/
	...
	DPoint3& operator=(const Point3& a) {	x = a.x; y = a.y; z = a.z;	return *this; }
	...
	DPoint3. */
	double operator*(const DPoint3&) const;		// DOT PRODUCT
	/*! \remarks Computes the cross product of this DPoint3 and the specified
	DPoint3. */
	DPoint3 operator^(const DPoint3&) const;	// CROSS PRODUCT
};
...
DPoint3 Normalize(const DPoint3&); // Return a unit vector.

DPoint3 operator*(double, const DPoint3&);	// multiply by scalar
DPoint3 operator*(const DPoint3&, double);	// multiply by scalar
DPoint3 operator/(const DPoint3&, double);	// divide by scalar
...
inline DPoint3 Normalize(const DPoint3& v)
{
	return v/Length(v);
}
...
DPoint3 CrossProd(const DPoint3& a, const DPoint3& b);	// CROSS PRODUCT
	
/*! \remarks Returns the dot product of two DPoint3s. */
double DotProd(const DPoint3& a, const DPoint3& b) ;		// DOT PRODUCT